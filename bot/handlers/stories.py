...
async def show_stories(message: Message, user=None, session=None) -> None:
    ...
    try:
        ...
        stories = await StoryService(StoryRepository(session)).get_user_stories(user.id)
        if not stories:
            kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➕ Добавить историю", callback_data="story:upload")]])
            await message.answer("📖 У тебя пока нет активных историй.\nИстории живут 24 часа.", reply_markup=kb)
            return
        buttons = []
        for s in stories:
            buttons.append([
                InlineKeyboardButton(text=f"👁 {s.view_count} просм. | {s.media_type}", callback_data=StoryCallback(action="view", story_id=s.id).pack()),
                InlineKeyboardButton(text="🗑", callback_data=StoryCallback(action="delete", story_id=s.id).pack()),
            ])
        buttons.append([InlineKeyboardButton(text="➕ Добавить историю", callback_data="story:upload")])
        await message.answer(f"📖 <b>Мои истории</b> ({len(stories)} активных):", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def story_upload_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer("📸 Отправь фото или видео для истории.\nИстория будет активна 24 часа.")
...
async def story_upload_photo(message: Message, state: FSMContext, user=None, session=None) -> None:
    ...
    try:
        from bot.services.story_service import StoryService
        from database.repositories.story_repository import StoryRepository
        success, msg = await StoryService(StoryRepository(session)).create_story(user.id, message.photo[-1].file_id, "photo", user.is_premium)
...
async def story_upload_video(message: Message, state: FSMContext, user=None, session=None) -> None:
    ...
    try:
        from bot.services.story_service import StoryService
        from database.repositories.story_repository import StoryRepository
        success, msg = await StoryService(StoryRepository(session)).create_story(user.id, message.video.file_id, "video", user.is_premium)
        await state.update_data(awaiting_story=False)
        await message.answer(f"{'✅' if success else '❌'} {msg}")
    except Exception as e:
...
async def story_view(callback: CallbackQuery, callback_data: StoryCallback, user=None, session=None) -> None:
    ...
    try:
        from bot.services.story_service import StoryService
        from database.repositories.story_repository import StoryRepository
        story = await StoryService(StoryRepository(session)).view_story(callback_data.story_id, user.id)
        if story.media_type == "photo":
            await callback.message.answer_photo(story.file_id, caption=f"👁 Просмотров: {story.view_count}")
        elif story.media_type == "video":
            await callback.message.answer_video(story.file_id, caption=f"👁 Просмотров: {story.view_count}")
    except ValueError:
