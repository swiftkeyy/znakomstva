...
async def swipe_start(message: Message, state: FSMContext, data: dict) -> None:
    user = data["user"]
    ...
    try:
        ...
        logger.info("candidate_shown", user_id=user.id, candidate_id=candidate.id, score=score)
    except Exception as e:
        logger.error("swipe_start_error", user_id=user.id, error=str(e))
        await message.answer("Не удалось загрузить анкеты. Попробуй позже.")


@router.callback_query(SwipeCallback.filter(F.action == "like"))
async def swipe_like(callback: CallbackQuery, callback_data: SwipeCallback, state: FSMContext, data: dict) -> None:
        ...
        if match:
            ...
            try:
                await callback.bot.send_message(
                    target_id,
                    f"🎉 <b>Взаимная симпатия!</b>\n{user.first_name} тоже лайкнул(а) тебя!",
                    parse_mode="HTML",
                )
            except Exception:
                pass
            logger.info("match_created_swipe", user_id=user.id, target_id=target_id)
        else:
            await callback.message.edit_reply_markup(reply_markup=None)

        # Load next candidate
        await swipe_start(callback.message, state, data)
    except Exception as e:
...
async def swipe_pass(callback: CallbackQuery, callback_data: SwipeCallback, state: FSMContext, data: dict) -> None:
    ...
    try:
        from database.repositories.swipe_repository import SwipeRepository
        swipe_repo = SwipeRepository(session)
        await swipe_repo.create_swipe(user.id, callback_data.user_id, "pass")
        await callback.message.edit_reply_markup(reply_markup=None)
        await swipe_start(callback.message, state, data)
    except Exception as e:
...
async def swipe_super_like(callback: CallbackQuery, callback_data: SwipeCallback, state: FSMContext, data: dict) -> None:
    user = data["user"]
    session = data["session"]
    ...
    try:
        ...
        if user.crystals < cfg.superswipe_cost:
            await callback.answer(
                f"Недостаточно кристаллов! Нужно {cfg.superswipe_cost} 💠",
                show_alert=True,
            )
            return

        await user_repo.spend_crystals(user.id, cfg.superswipe_cost)
        from database.repositories.swipe_repository import SwipeRepository
        ...
        try:
            await callback.bot.send_message(
                ...
                f"⭐ <b>{user.first_name}</b> отправил(а) тебе SuperSwipe!",
                parse_mode="HTML",
            )
        except Exception:
            pass
        ...
        await callback.message.edit_reply_markup(reply_markup=None)
        await swipe_start(callback.message, state, data)
        logger.info("super_like_sent", user_id=user.id, target_id=callback_data.user_id)
    except Exception as e:
...
async def swipe_write(callback: CallbackQuery, callback_data: SwipeCallback, data: dict) -> None:
    await callback.answer()
    user = data["user"]
    session = data["session"]
    ...
    try:
        from database.repositories.match_repository import MatchRepository
        match_repo = MatchRepository(session)
        match = await match_repo.get_match(user.id, callback_data.user_id)

        if match is None:
            await callback.message.answer(
                "💬 Чтобы написать напрямую, нужна взаимная симпатия. Сначала поставь лайк!"
            )
            return
        ...
        await callback.message.answer(
            ...
            reply_markup=chat_keyboard(match.id),
        )
        logger.info("write_initiated", user_id=user.id, target_id=callback_data.user_id)
    except Exception as e:
...
async def toggle_deep_search(message: Message, state: FSMContext, data: dict) -> None:
    fsm_data = await state.get_data()
    ...
    logger.info("deep_search_toggled", user_id=data["user"].id, enabled=deep)
